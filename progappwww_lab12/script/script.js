...
function rememberSelectedItem(list) {
  ...
  listElements.forEach(option => {
    switch(list.name)
    {
      ...
      break;
      case "products":
        if (params.get("products") == option.value)
        list.value = option.value;
      break;
    }
  });
...
$(document).on('keyup', function (e) {
  ...
  if (e.key == "Escape") $('#close-cart-mark').click();
});


var cartVisibible = false;
...
function showCart()
{
  ...
  {
    cartVisibible = true;
    let closeCartMark = $('#close-cart-mark');
    closeCartMark.css("display", "block");
    let cartDiv = $('#cart-menu');
    cartDiv.css("display", "block");
    $.ajax({
      type: "GET",
      url: "server_script/showCart.php",
      success: function (data) {
        cartDiv.html(data);
    }
  });
...
function addToCart(id)
{
        $.ajax({
            type: "GET",
            url: "server_script/addItemToCart.php",
            data: {
                'id': id
            },
            success: function()
...
function closeCart() {
  ...
  let closeCartMark = $('#close-cart-mark');
  closeCartMark.css("display", "none");
}
...
function updateQuantity(id, val)
{
  $.ajax({
    type: "GET",
    url: "server_script/updateQuantity.php",
    data: {
      'id': id,
      'val': val
    },
...
function removeProduct(id)
{
  $.ajax({
    type: "GET",
    url: "server_script/removeProduct.php",
    data: {
      'id': id
    },
    success: function()
...
function saveCart()
{
  ...
  $.ajax({
    type: "GET",
    url: "server_script/saveCart.php",
    success: function (data) {
      warning.html(data);
  }
});
